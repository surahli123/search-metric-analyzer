...
export default function CoMovementIndicator({ coMovement }) {
  ...
  return (
    <div>
      ...
      >
        {/* Conclusion sentence — tells the reader what the pattern means BEFORE they see the metrics */}
        <div className="flex items-center gap-2 mb-3 text-sm" style={{ color: 'var(--text-secondary)' }}>
          {is_positive ? (
            <>
              {/* Green checkmark icon for expected patterns */}
              <span
              ...
              <span>All metrics moving as expected. When AI answers improve, users click less — that's normal.</span>
            </>
          ) : (
            <>
              {/* Amber warning icon for unexpected patterns */}
              <span
                className="inline-flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold flex-shrink-0"
              ...
              <span>Unusual metric pattern detected.</span>
            </>
          )}
        </div>
