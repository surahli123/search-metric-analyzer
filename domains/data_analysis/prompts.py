...
def build_hypothesize_system_prompt() -> str:
    ...
    return (
        ...
        "- SQL must be valid SQLite/DuckDB syntax\n"
        ...
        "- If the question asks for a specific format, note that in the description"
    )
...
def build_synthesize_system_prompt() -> str:
    ...
    return (
        ...
        "- Include a header row that matches the question's expected output columns\n"
        "- If the result is a single value, output just the header and value\n"
        "- If the result has multiple rows, output all rows\n"
        "- Numbers should be plain (no formatting, no currency symbols)\n"
        "- Strings should not be quoted unless they contain commas\n\n"
        "EXAMPLES:\n"
